...
(function() {
    ...
    const utils = {
        keys(obj) {
            var result = [], index = 0;
            for (var key in obj)
                obj != null && (result[index] = key, index++);
            return result;
        },
        values(obj) {
            var index = 0;
            var keys = this.keys(obj), result = [], i = 0, j = 0;
            while (i < keys.length) {
    ...
    const gameUtils = {
        ...
        get doAttack() {
            let heldItem = this.safeGetHeldItem(1), breakingItem = heldItem?.breakingItem;
        ...
        placeBlock(position) {
            ...
            if (!obj1) return;
            const key2 = Object.keys(obj1).find(k => obj1[k] && obj1[k].position) || Object.keys(obj1)[25];
    ...
    function setupTemporaryBindHijack() {
        if (moduleLoader || gameState.injected) return;
    ...
    function inject() {
        try {
            ...
            if (!noaObj) return false;
            const noaValues = utils.values(noaObj);
            gameState.noa = noaObj;
            gameState.bloxd = noaObj.bloxd;
            gameState.physics = noaObj.physics;
            gameState.world = noaValues.find(v => v && v.chunkStorage) || noaValues[11];
            gameState.rendering = noaValues.find(v => v && typeof v.getScene === 'function' || (v && v.scene)) || noaValues[12];
            gameState.playerData = noaValues.find(v => v && typeof v.walkingSpeed === 'number') || noaValues[29];
            gameState.entityList = noaValues.find(v => v && v[1] && v[1][1] && v[1][1].lobbyLeaderboardValues) || noaValues[30];
            ...
            if (ui) {
                ui.notify('Ready (Right Shift to toggle menu.)', '#36A3FF');
            }
            enableModules();
            return true;
        } catch (err) {
    ...
    function setupInjectionObserver() {
        const checkInjection = () => {
            ...
            if (found) {
                ...
                if (!injected && inject()) {
                    injected = true;
                    console.log("Selene: Injection Successful");
                    observer.disconnect(); // Ngắt observer cho đỡ lag
                }
            } else {
                injected = false;
            }
    ...
    const ui = {
        ...
        shadowRoot: null,
        menuButton: null,
        categories: ['Combat', 'Movement', 'Visuals', 'World', 'HUD'],
        init() {
        ...
        injectStyles() {
            ...
            style.innerHTML = `
                ...
                .bone-dropdown { background: rgba(0,0,0,0.25); display: flex; flex-direction: column; gap: 8px; max-height: 0; opacity: 0; overflow: hidden; padding: 0 12px; transition: max-height 0.3s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.2s ease, padding 0.3s ease, background-color 0.3s; }
                .bone-dropdown.open { max-height: 400px; opacity: 1; padding: 10px 12px; border-bottom: 1px solid rgba(255,255,255,0.02); }
                ...
                .bone-notify-box { position: fixed; bottom: 25px; right: 25px; z-index: 10001; display: flex; flex-direction: column; align-items: flex-end; gap: 0; font-family: var(--font-main); pointer-events: none; }
                .bone-toast-wrapper { overflow: hidden; opacity: 0; max-height: 0; margin-top: 0; transform: translateX(120%); transition: max-height 0.35s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.35s ease, transform 0.35s cubic-bezier(0.4, 0, 0.2, 1), margin-top 0.3s ease; }
                .bone-toast-wrapper.show { opacity: 1; max-height: 60px; margin-top: 10px; transform: translateX(0); }
                .bone-toast-wrapper.exit { opacity: 0; max-height: 0; margin-top: 0; transform: translateX(120%); }
                .bone-toast { padding: 10px 18px; background: var(--bg-panel); border-radius: 6px; color: #fff; font-size: 12px; border: 1px solid var(--border); border-left: 3px solid var(--primary); box-shadow: 0 5px 15px rgba(0,0,0,0.3); white-space: nowrap; }
                .bone-watermark { position: fixed; top: 15px; left: 15px; font-family: var(--font-main); font-size: 20px; font-weight: 700; color: var(--primary); text-shadow: 2px 2px 4px rgba(0,0,0,0.5); z-index: 9998; pointer-events: none; display: block; }
        ...
        createHUD() {
            ...
            this.hudInfo.innerHTML = '<span id="bone-fps">FPS: 0</span><span id="bone-ping">Ping: 0</span>';
            ...
            this.keystrokes.innerHTML = `
                ...
                <div class="bone-key mouse" id="key-lmb">LMB<div class="bone-cps" id="cps-lmb">0 CPS</div></div>
                <div class="bone-key mouse" id="key-rmb">RMB<div class="bone-cps" id="cps-rmb">0 CPS</div></div>
                </div>
                <div class="bone-key-row"><div class="bone-key spacebar" id="key-space">SPACE</div></div>
                `;
        ...
        updateHUD() {
            if (!this.arrayList || !this.watermark) return;
            
            const arrayListModule = client.modules['Module List'];
            const blurModule = client.modules['Background Blur'];
            ...
            if (!arrayListModule || !arrayListModule.enabled) return;
            ...
            this.categories.forEach(category => {
                ...
                modules.forEach(module => {
                    ...
                    moduleEl.onclick = event => {
                        if (event.target.classList.contains('bone-module-expand') || this.bindingModule) return;
                        if (module.type === 'action') {
                            module.onEnable?.();
                            ui.notify(`${module.name} Executed`, '#2F8CFF');
        ...
        setupEvents() {
            ...
            window.addEventListener('contextmenu', event => {
                const fastPlace = client.modules['Fast Place'];
            ...
            window.addEventListener('mouseup', event => {
                if (event.button === 0) {
                    const lmbKey = this.shadowRoot.getElementById('key-lmb');
                    if (lmbKey) lmbKey.classList.remove('active');
                } else if (event.button === 2) {
                    const rmbKey = this.shadowRoot.getElementById('key-rmb');
                    if (rmbKey) rmbKey.classList.remove('active');
                }
            });
            ...
            window.addEventListener('keydown', event => {
                ...
                if (this.bindingModule) {
                    ...
                    this.notify(`${module.name} bound to ${module.key || 'None'}`);
                    return;
                }
        ...
        notify(message, color) {
            ...
            const isModule = message.includes("Enabled") || message.includes("Disabled");
            const bgColor = isModule ? menuColor : catColor;

            const isSpecialNotification = message === "Ready (Right Shift to toggle menu.)" || message === "New Account Executed" || message.includes("bound to ");
    ...
    class Module {
        ...
        setLoop(callback) {
            ...
            const tick = () => {
                if (!this.enabled || !loop.active) return;
                callback();
        ...
        toggle(silent=false) {
            ...
            if (this.enabled) {
                this._intervals = [];
                this._loops = [];
                if (gameState.injected) this.onEnable?.();
            } else {
                ...
                this._intervals = [];
                this._loops = [];
                if (gameState.injected) this.onDisable?.();
            }
            if (!silent && ui) {
                ui.notify(`${this.name} ${this.enabled ? 'Enabled' : 'Disabled'}`, this.enabled ? '#36A3FF' : '#FF4D6D');
            }
        }
    }

    function getThinMeshes() {
        if (!gameState.rendering) return [];
        if (gameState.rendering.thinMeshes) return gameState.rendering.thinMeshes;
        const renderingValues = utils.values(gameState.rendering);
        for (const val of renderingValues) {
            if (val && val.thinMeshes && Array.isArray(val.thinMeshes)) return val.thinMeshes;
        }
        return [];
    }
    ...
    function setRenderingGroup(enabled) {
        ...
        for (const mesh of meshes) {
            ...
            if (defaultMesh) defaultMesh.renderingGroupId = enabled ? 2 : 0;
        }
    }

    function worldToScreen(position) {
        if (!gameState.Lion?.scene) return null;
    ...
    function createMesh(name, texture, position) {
        if (!gameState.Lion || !gameState.noa) return null;
        try {
    ...
    function disposeMesh(obj) {
        ...
        try {
            ...
            if (obj.entityId) gameState.noa.entities.deleteEntity(obj.entityId);
        } catch (err) {}
    }

    function scanChunks(module, blockIds, callback) {
        if (!gameState.world || !gameState.impKey || !gameState.world[gameState.impKey]?.hash) return;
        ...
        for (const key in chunks) {
            ...
            if (dist < 96) {
                nearbyChunks.push({ chunk: chunk, dist: dist });
            }
        }
        ...
        nearbyChunks.forEach( ({chunk}) => {
            ...
            if (module.scannedChunks.has(chunkKey)) return;
    ...
    function findNearestBlock(position) {
        ...
        EXyA: for (const offset of neighborOffsets) {
            if (offset.dist > 64) break EXyA;
            const [nx,ny,nz] = [x + offset.x, y + offset.y, z + offset.z];
            if (gameUtils.getBlockID(nx, ny, nz) !== 0) return [nx, ny, nz];
        }
        return null;
    }
    ...
    const keysToReverse = {
        KeyW: { set: 'backward', clear: 'forward' },
        KeyS: { set: 'forward', clear: 'backward' },
        KeyA: { set: 'right', clear: 'left' },
        KeyD: { set: 'left', clear: 'right' },
        ArrowUp: { set: 'backward', clear: 'forward' },
        ArrowDown: { set: 'forward', clear: 'backward' },
        ArrowLeft: { set: 'right', clear: 'left' },
        ArrowRight: { set: 'left', clear: 'right' }
    };

    function startKeyReversal() {
        if (keyReversalActive) return;
    ...
    function initKeyReversalCheck() {
        if (keyReversalInterval) return;
        keyReversalInterval = setInterval( () => {
            const flyEnabled = client.modules['Fly']?.enabled || false;
            const speedEnabled = client.modules['Speed']?.enabled || false;
    ...
    function registerModules() {
        client.modules['Kill Aura'] = new Module('Kill Aura','Combat',{
            lastAttackTime: 0,
            settings: {
                'Delay': { type: 'slider', min: 0, max: 1000, step: 1, value: 0 },
                'Range': { type: 'slider', min: 1, max: 10, step: 0.1, value: 10 },
                'Knockback': { type: 'slider', min: 0, max: 5, step: 0.1, value: 5 },
                'Pull': { type: 'checkbox', value: false },
                'Swing Animation': { type: 'checkbox', value: true },
                'Target Mobs': { type: 'checkbox', value: false },
                'Criticals': { type: 'checkbox', value: true }
            },
            onEnable: function() {
            ...
            onTick: function() {
                const now = Date.now();
                const delay = this.settings['Delay'].value;

                if (now - this.lastAttackTime < delay) return;

                const includeMobs = this.settings['Target Mobs'].value;
                const swing = this.settings['Swing Animation'].value;
                const pullEnable = this.settings['Pull'].value;
                const kbStrength = this.settings['Knockback'].value;
                const detectionRange = this.settings['Range'].value;

                const targets = includeMobs ? gameUtils.entityList : gameUtils.playerList;
                const playerPos = gameUtils.getPosition(1);

                if (!playerPos || !targets) return;
                ...
                targets.forEach(entityId => {
                    ...
                    if (lifeState && !lifeState.isAlive) return;

                    targetArr.push({
                        id: entityId, pos: targetPos, distSq: distSq
                    });
                });

                if (targetArr.length === 0) return;

                const useCrit = this.settings['Criticals'].value;
                if (useCrit) {
                    const moveState = gameUtils.getMoveState(1);
                    if (moveState && moveState.isOnGround) {
                        moveState.jumping = true;
                    } else if (gameUtils.simulateJump) {
                ...
                targetArr.forEach(target => {
                    try {
                        let direction = [0, 0, 0];
                        let dx = target.pos[0] - playerPos[0];
                        let dy = target.pos[1] + 1 - (playerPos[1] + 1.5); 
                        let dz = target.pos[2] - playerPos[2];
                        ...
                        direction[0] *= kbStrength; direction[1] *= kbStrength; direction[2] *= kbStrength;

                        gameUtils.doAttack(direction, String(target.id), 'BodyMesh');
                    } catch (err) {}
                });
                ...
                if (swing) {
                    try {
                        ...
                        playerEntity?._standardItem?.trySwingBlock?.(now);
                    } catch(e) {}
                }
            }
        });

        client.modules['Auto Potion'] = new Module('Auto Potion','Combat',{
            lastSwapTime: 0,
            settings: { 'Target Slot': { type: 'slider', min: 0, max: 8, step: 1, value: 8 } },
            onEnable: function() {
                this.setInterval( () => {
                    if (Date.now() - this.lastSwapTime < 500) return;
                    try {
                        const inventory = gameState.noa.entities[gameState.impKey]?.inventory?.list?.[0]?.opWrapper;
                        if (!inventory || !inventory.playerInventory?.items) return;
                        const items = inventory.playerInventory.items;
                        const targetSlot = this.settings['Target Slot'].value;
                        const isPotion = item => item?.name && (item.name.toLowerCase().includes('potion') || item.name.toLowerCase().includes('heal'));
                        if (isPotion(items[targetSlot])) return;
                        let potionSlot = -1;
        ...
        client.modules['TriggerBot'] = new Module('TriggerBot','Combat',{
            lastTriggerTime: 0,
            onEnable: function() {
                this.setLoop( () => {
                    const playerEntity = gameUtils.getPlayerEntity();
                    ...
                    if (result?.hitEId && lifeState?.isAlive) {
                        const now = Date.now();
                        if (!this.lastTriggerTime || now - this.lastTriggerTime > 150) {
                            this.lastTriggerTime = now;
                            const heading = gameState.noa.camera.heading, pitch = gameState.noa.camera.pitch;
        ...
        client.modules['Hitbox Extender'] = new Module('Hitbox Extender','World',{
            originalAABB: null,
            aabbKey: null,
            onEnable: function() {
        ...
        client.modules['Magic Bullet'] = new Module('Magic Bullet','Combat',{
            _OriginalFireBullet: null,
            onEnable: function() {
                const self = this;
                this.setInterval( () => {
                    const gunItem = gameUtils.getPlayerEntity()?._gunItem;
                    if (!gunItem || !gunItem.fireBullet) return;
                    ...
                    if (gunItem.fireBullet === this._OriginalFireBullet) {
                        gunItem.fireBullet = function(...args) {
                            ...
                            if (target) {
                                ...
                                if (args[0] && typeof args[0].constructor === 'function') {
                                    args[0] = new args[0].constructor(dx / dist, dy / dist, dz / dist);
                                } else {
        ...
        client.modules['Auto Trap'] = new Module('Auto Trap','Combat',{
            settings: { 'Range': { type: 'slider', min: 1, max: 12, step: 0.1, value: 12 } },
            onEnable: function() {
                this.setInterval( () => {
                    const heldItem = gameUtils.safeGetHeldItem(1);
                    if (!heldItem?.typeObj?.name) return;
                    const itemName = heldItem.typeObj.name;
                    if (!itemName.includes('Net') && !itemName.includes('Spike')) return;
                    ...
                    targets.forEach(entityId => {
                        ...
                        const dz = targetPos[2] - playerPos[2];
                        const distSq = dx * dx + dy * dy + dz * dz;
                        if (distSq > rangeHalfSq) return;
                        const lifeState = gameState.noa.entities.getState(entityId, 'genericLifeformState');
                        if (lifeState && !lifeState.isAlive) return;
                        targetArr.push({
                            id: entityId, pos: targetPos, dist: Math.hypot(dx, dy, dz)
                        });
                    });
        ...
        client.modules['Fly'] = new Module('Fly', 'Movement', {
            settings: { 'Speed Boost': { type: 'slider', min: 1, max: 100, step: 1, value: 25 } },
            _origGravity: null, _origWalkSpeed: null, _origCrouchSpeed: null, _origSprintSpeed: null,
            onEnable: function() { this._saveOriginalValues(); },
            onDisable: function() { this._restoreValues(); },
            onTick: function() {
            ...
            _saveOriginalValues: function() {
                if (gameState.physics) {
                    ...
                    for (const key in phys) {
                        const val = phys[key];
                        if (Array.isArray(val) && val.length === 3 && typeof val[0] === 'number') {
                            this._origGravity = { obj: phys, key: key, value: val[1] };
                            break;
                        }
                    }
                }
            ...
            _applyFly: function() {
                if (this._origGravity && this._origGravity.obj[this._origGravity.key]) {
                    this._origGravity.obj[this._origGravity.key][1] = 0; 
                }
                const boost = this.settings['Speed Boost'].value;
                const body = gameUtils.getPhysicsBody(1);
                const inputState = gameState.noa?.inputs?.state;
                if (body && body.velocity) {
                    if (inputState?.jump) {
                        body.velocity[1] = boost / 2;
                ...
                if (gameState.playerData) {
                    gameState.playerData.walkingSpeed = boost;
                    gameState.playerData.sprintingSpeed = boost * 1.5;
                    gameState.playerData.crouchingSpeed = boost;
                }
            },
            _restoreValues: function() {
                if (this._origGravity && this._origGravity.obj[this._origGravity.key]) {
                    this._origGravity.obj[this._origGravity.key][1] = this._origGravity.value;
                }
                if (gameState.playerData) {
                    if (this._origWalkSpeed != null) gameState.playerData.walkingSpeed = this._origWalkSpeed;
                    if (this._origCrouchSpeed != null) gameState.playerData.crouchingSpeed = this._origCrouchSpeed;
                    if (this._origSprintSpeed != null) gameState.playerData.sprintingSpeed = this._origSprintSpeed;
                }
            }
        });

        client.modules['Speed'] = new Module('Speed','Movement',{
            settings: { 'Movement Speed': { type: 'slider', min: 1, max: 30, step: 1, value: 2 } },
            _originalWalkSpeed: null, _originalCrouchSpeed: null, _originalSprintSpeed: null,
            onEnable: function() {
                if (gameState.playerData) {
            ...
            onTick: function() {
                if (!this.enabled || !gameState.playerData) return;
                const speedVal = this.settings['Movement Speed'].value;
                const inputState = gameState.noa?.inputs?.state;
                let mul = 4;
                if (inputState?.crouch) mul = 2;
                else if (inputState?.sprint) mul = 6;
        ...
        client.modules['Scaffold'] = new Module('Scaffold','Movement',{
            isBuilding: false, interval: null, lastPosition: null, lastVelocity: [0, 0, 0], airTicks: 0, lastPlaceTime: 0,
            onEnable: function() { this.startInterval(); },
            onDisable: function() {
                if (this.interval) { this.clearInterval(this.interval); }
                this.interval = null; this.lastPosition = null; this.lastVelocity = [0, 0, 0]; this.airTicks = 0;
            },
            startInterval: function() {
                if (this.interval) { this.clearInterval(this.interval); }
                const delay = 1;
                this.interval = this.setInterval( () => {
                    if (this.enabled) { this.tryPlaceBlock(); }
                }, delay);
                this.tryPlaceBlock();
            },
            checkFeetBlock: function(pos) {
                if (!pos) return false;
                const x = Math.floor(pos[0]), y = Math.floor(pos[1] - 1), z = Math.floor(pos[2]);
            ...
            findNearestBlockInRadius: function(pos, radius=7) {
                ...
                let nearestBlock = null, minDistance = Infinity;
            ...
            generatePathToPlayer: function(start, end) {
                if (!start || !end) return [];
                const path = [];
                const ex = Math.floor(end[0]), ey = Math.floor(end[1] - 1), ez = Math.floor(end[2]);
                let x = start[0], y = start[1], z = start[2];
                while (y > ey) {
                    y--;
                    const blockID = gameUtils.getBlockID(x, y, z);
                    if (!blockID || blockID === 0) { path.push([x, y, z]); }
                }
                while (x !== ex || z !== ez) {
                    if (x !== ex) {
                        x += x < ex ? 1 : -1;
                ...
                if (!hasEndPos) { path.push([ex, ey, ez]); }
                return path;
            },
            ...
            tryPlaceBlock: function() {
                try {
                    ...
                    if (!playerEntity?._blockItem || playerEntity._blockItem.typeObj?.type !== "CubeBlock") {
                        this.isBuilding = false; return;
                    }
                    const playerPos = gameUtils.getPosition(1);
                    if (!playerPos) { this.isBuilding = false; return; }
                    if (this.checkFeetBlock(playerPos)) {
                        this.airTicks = 0; this.isBuilding = false; return;
                    }
                    const nearestBlock = this.findNearestBlockInRadius(playerPos, 7);
                    if (!nearestBlock) {
                        this.airTicks++; this.isBuilding = false; return;
                    }
                    ...
                    uniquePath.forEach(pos => {
                        const [x,y,z] = pos;
                        const blockID = gameUtils.getBlockID(x, y, z);
                        if (!blockID || blockID === 0) { this.placeAt(x, y, z); }
                    });
                    this.lastPlaceTime = Date.now();
                    this.airTicks = 0;
                    this.isBuilding = false;
                } catch { this.isBuilding = false; }
            },
            placeAt: function(x, y, z) {
                if (this.canPlaceBlock(x, y, z)) {
                    gameUtils.placeBlock([x, y, z]);
                    this.airTicks = 0; this.lastPlaceTime = Date.now();
                    return true;
                }
                return false;
            },
        ...
        client.modules['Bhop'] = new Module('Bhop', 'Movement', {
            ...
            onEnable: function() {
                ...
                // Lắng nghe thao tác giữ phím Space của người chơi
                this._kd = (e) => { 
                    if(e.code === 'Space' && !e.repeat) this._spacePressed = true; 
                };
                this._ku = (e) => { 
                    if(e.code === 'Space') {
                        this._spacePressed = false;
                        // Nhả phím nhảy trong game khi người dùng nhả Space
                        if (gameState.noa?.inputs?.state) gameState.noa.inputs.state.jump = false;
                    }
                };
                this._bl = () => { 
                    this._spacePressed = false; 
            ...
            onTick: function() {
                // Chỉ hoạt động khi người chơi đang giữ Space
                if (!this._spacePressed) return;

                const physState = gameUtils.getPhysState(1);
                const inputState = gameState.noa?.inputs?.state;
                
                if (!physState || !inputState) return;

                // Kiểm tra xem nhân vật có đang chạm đất không
                let onGround = false;
                try {
                    onGround = typeof physState.isOnGround === 'function' ? physState.isOnGround() : false;
                } catch(e) {}

                if (onGround) {
                    // Chạm đất -> Ép nhảy ngay lập tức (Không delay, không giới hạn)
                    inputState.jump = true;
                } else {
                    // Đang trên không -> Tự động nhả phím nhảy để chuẩn bị cho nhịp đáp đất tiếp theo
                    inputState.jump = false;
                }
            }
        });

        client.modules['No Slowdown'] = new Module('No Slowdown','Movement',{
            onEnable: () => { if (gameState.playerData) gameState.playerData.crouchingSpeed = 7; },
            onDisable: () => { if (gameState.playerData) gameState.playerData.crouchingSpeed = 2; }
        });

        client.modules['Auto Sprint'] = new Module('Auto Sprint','Movement',{
            onEnable: () => { if (gameState.playerData) gameState.playerData.walkingSpeed = 7; },
            onDisable: () => { if (gameState.playerData) gameState.playerData.walkingSpeed = 6; }
        });

        client.modules['Nametags'] = new Module('Nametags','Visuals',{
            onEnable: function() { this.lastUpdate = 0; },
            onTick: function() {
                let now = Date.now();
                if (now - (this.lastUpdate || 0) < 3000) return;
                this.lastUpdate = now;
                if (!gameState.entityList || !gameState.Lion?.scene) return;
                    ...
                    if (entities) {
                        for (let entity of Object.values(entities)) {
                            if (entity?.lobbyLeaderboardValues) {
                                Object.defineProperty(entity, 'hasPriorityNametag', { get: () => true, set(val) {}, configurable: true });
                                Object.defineProperty(entity, 'canSee', { get: () => true, set(val) {}, configurable: true });
                            }
                        }
                    }
                }
                gameState.Lion.scene.meshes.forEach(mesh => {
                    if (mesh?.id?.includes?.('NameTag')) {
                        Object.defineProperty(mesh, '_isVisible', { get: () => true, set(val) {}, configurable: true });
                        Object.defineProperty(mesh, 'renderingGroupId', { get: () => 3, set(val) {}, configurable: true });
                    }
                });
            }
        });

        client.modules['Custom FOV'] = new Module('Custom FOV','Visuals',{
            _baseFOV: 160 / 83.33333333333333333333, _originalGetViewMatrix: null, _camera: null,
            settings: {
                'FOV': { type: 'slider', min: 60, max: 160, step: 1, value: 160, onChange: function(fov) {
                    client.modules['Custom FOV']._baseFOV = fov / 8.333333333333333333333;
                } }
            },
            onEnable: function() {
                this._baseFOV = this.settings['FOV'].value / 83.33333333333333333333;
                this._hookViewMatrix();
            },
            _hookViewMatrix: function() {
                if (!gameState.Lion?.scene?.activeCamera) {
                    setTimeout( () => this._hookViewMatrix(), 100); return;
                }
                this._camera = gameState.Lion.scene.activeCamera;
                if (!this._originalGetViewMatrix) { this._originalGetViewMatrix = this._camera._getViewMatrix; }
                const self = this;
            ...
            onTick: function() {
                if (!this.enabled) return;
                if (gameState.Lion?.scene?.activeCamera !== this._camera) { this._hookViewMatrix(); }
                if (this._camera) { this._camera.fov = this._baseFOV; }
            },
            onDisable: function() {
                ...
                this._camera = null; this._originalGetViewMatrix = null;
            }
        });

        client.modules['Coordinates'] = new Module('Coordinates','Visuals',{
            onEnable: function() {
                this.setInterval( () => {
        ...
        client.modules['Night Mode'] = new Module('Night Mode','Visuals',{
            _origClearColor: null, _skyboxMesh: null,
            _findSkybox: function() {
                if (!gameState.noa) return null;
            ...
            onEnable: function() { this._origClearColor = null; this._skyboxMesh = null; },
            onTick: function() {
                try {
                    ...
                    if (scene) {
                        if (!this._origClearColor && scene.clearColor) {
                            this._origClearColor = { r: scene.clearColor.r, g: scene.clearColor.g, b: scene.clearColor.b };
                        }
                        if (scene.clearColor) {
                            scene.clearColor.r = 0.05; scene.clearColor.g = 0.05; scene.clearColor.b = 0.1;
                        }
                        if (scene.fogMode !== undefined) scene.fogMode = 0;
                    }
                    this._skyboxMesh = this._findSkybox();
                    if (this._skyboxMesh) { this._skyboxMesh.isVisible = false; }
                } catch (e) {}
            },
            onDisable: function() {
                try {
                    ...
                    if (scene && this._origClearColor && scene.clearColor) {
                        scene.clearColor.r = this._origClearColor.r; scene.clearColor.g = this._origClearColor.g; scene.clearColor.b = this._origClearColor.b;
                    }
                    if (this._skyboxMesh) { this._skyboxMesh.isVisible = true; this._skyboxMesh = null; }
                    this._origClearColor = null;
                } catch (e) {}
            }
        });

        client.modules['Tracers'] = new Module('Tracers','Visuals',{
            _canvas: null,
            onEnable: function() {
            ...
            onTick: function() {
                try {
                    ...
                    ctx.lineWidth = 1.5; ctx.strokeStyle = '#ff0000'; ctx.shadowColor = '#ff0000'; ctx.shadowBlur = 8;

                    const playerList = gameUtils.playerList;
                    if (!playerList || playerList.length === 0) { ctx.restore(); return; }
        ...
        client.modules['Animations'] = new Module('Animations','Visuals',{
            originalSetFirstPersonMeshPosition: null,
            onEnable: function() {
                const playerEntity = gameUtils.getPlayerEntity();
                if (!playerEntity?._standardItem) return;
                const itemProto = Object.getPrototypeOf(playerEntity._standardItem);
                if (!itemProto || !itemProto.setFirstPersonMeshPosition) return;
                ...
                itemProto.setFirstPersonMeshPosition = function(time) {
                    if (self.enabled) {
                        const mesh = this.firstPersonMesh, state = this.heldItemState;
                        if (!mesh || !state) return;
                        const Vector3 = mesh.position.constructor;
                        const x = 0.23 + this.firstPersonPosOffset.x, y = -0.2 + this.firstPersonPosOffset.y, z = 0.3 + this.firstPersonPosOffset.z;
                        const rotation = new Vector3(0.4,0.7,0.3).add(this.firstPersonRotation);
                        const swingTime = time - state._lastSwingStart, swingDuration = state.swingDuration;
                        if (swingTime < swingDuration) {
                            const progress = Math.sin(Math.pow(swingTime / swingDuration, 0.35) * Math.PI);
                            const a = 0.26, b = 1.07;
                            const rx = -0.3 * progress, ry = -0.4 * progress, rz = 0.72 * progress;
                            ...
                            const dy = (Math.cos(rotation.z + rz + b) - Math.cos(rotation.z + b)) * a;
                            mesh.position.x = x - dx; mesh.position.y = y + dy; mesh.position.z = z;
                        } else {
                            mesh.rotation = rotation; mesh.position = new Vector3(x,y,z);
                        }
            ...
            onDisable: function() {
                ...
                if (this.originalSetFirstPersonMeshPosition && playerEntity?._standardItem) {
                    const itemProto = Object.getPrototypeOf(playerEntity._standardItem);
                    if (itemProto) itemProto.setFirstPersonMeshPosition = this.originalSetFirstPersonMeshPosition;
                }
                this.originalSetFirstPersonMeshPosition = null;
            }
        });

        client.modules['Item Spin'] = new Module('Item Spin','Visuals',{
            originalSetFirstPersonMeshPosition: null,
            onEnable: function() {
                const playerEntity = gameUtils.getPlayerEntity();
                if (!playerEntity?._standardItem) return;
        ...
        client.modules['ESP'] = new Module('ESP','Visuals',{
            onEnable: () => {
                if (!espInterval) {
        ...
        client.modules['Background Blur'] = new Module('Background Blur','HUD',{
            enabled: true
        });

        client.modules['Target HUD'] = new Module('Target HUD','Visuals',{
            hudEl: null, nameEl: null, barEl: null, hpTextEl: null,
            onEnable: function() {
                if (!this.hudEl) {
            ...
            onTick: function() {
                ...
                const playerPos = gameUtils.getPosition(1);
                if (!playerPos) {
                    this.hudEl.style.display = 'none'; return;
                }
                const players = gameUtils.playerList;
                let target = null, minDist = Infinity;
                wRmA: for (const playerId of players) {
                    const lifeState = gameState.noa.entities.getState(playerId, 'genericLifeformState');
                    if (lifeState && !lifeState.isAlive) continue wRmA;
                    const targetPos = gameUtils.getPosition(playerId);
                    if (!targetPos) continue wRmA;
                    const dist = Math.hypot(targetPos[0] - playerPos[0], targetPos[1] - playerPos[1], targetPos[2] - playerPos[2]);
                    if (dist < minDist && dist < 30) {
                        minDist = dist; target = { id: playerId, pos: targetPos };
                    }
                }
                if (!target) {
                    this.hudEl.style.display = 'none'; return;
                ...
                let hpBarColor;
                if (healthPercent < 25) { hpBarColor = '#962D2D'; } else if (healthPercent < 50) { hpBarColor = '#D99A48'; } else { hpBarColor = '#36A37A'; }
        ...
        client.modules['Module List'] = new Module('Module List','HUD',{
            enabled: true,
            onEnable: () => { if (ui.arrayList) ui.arrayList.style.display = 'flex'; },
            onDisable: () => { if (ui.arrayList) ui.arrayList.style.display = 'none'; }
        });

        client.modules['FPS & Ping'] = new Module('FPS & Ping','HUD',{
            enabled: false,
            onEnable: () => { if (ui.hudInfo) ui.hudInfo.style.display = 'flex'; },
            onDisable: () => { if (ui.hudInfo) ui.hudInfo.style.display = 'none'; },
            onTick: function() {
                if (!ui.hudInfo) return;
                let ping = 0;
                ...
                ui.shadowRoot.getElementById('bone-ping').textContent = `Ping: ${ping}ms`;
            }
        });

        client.modules['Keystrokes'] = new Module('Keystrokes','HUD',{
            enabled: false,
            onEnable: () => { if (ui.keystrokes) ui.keystrokes.style.display = 'flex'; },
            onDisable: () => { if (ui.keystrokes) ui.keystrokes.style.display = 'none'; },
            onTick: function() {
                if (!ui.keystrokes) return;
                const now = Date.now();
        ...
        client.modules['Fast Place'] = new Module('Fast Place','World',{
            lastPlace: 0,
            settings: { 'Delay': { type: 'slider', min: 0, max: 100, step: 10, value: 0 } },
            onTick: function() {
                if (!document.isRightMouseDown) return;
                const now = Date.now();
                if (now - this.lastPlace >= this.settings['Delay'].value) {
        ...
        client.modules['Fast Break'] = new Module('Fast Break','World',{
            originalHardness: {}, applied: false,
            onEnable: function() {
                this.setInterval( () => {
                    ...
                    if (Object.keys(this.originalHardness).length === 0) {
                        blocks.forEach( (block, index) => {
                            if (block?.ttb) this.originalHardness[index] = block.ttb;
                        });
                    }
                    blocks.forEach( (block, index) => {
                        if (block?.ttb && this.originalHardness[index]) block.ttb = this.originalHardness[index] / 2;
                    });
                    this.applied = true;
                }, 500);
            },
            onDisable: function() {
                if (this.applied) {
                    ...
                    if (gunClass) {
                        ...
                        const blocks = Object.values(Object.values(moduleLoader(gunKey)).find(val => typeof val === 'object'));
                        blocks.forEach( (block, index) => {
                            if (block?.ttb && this.originalHardness[index]) block.ttb = this.originalHardness[index];
                        });
                    }
                }
        ...
        client.modules['Bed Aura'] = new Module('Bed Aura','World',{
            bedPos: null, pathCache: [], isBreaking: false,
            onEnable() { this.setInterval( () => this.onTick(), 200); },
            onDisable() {
                this.isBreaking = false; this.pathCache = []; this.bedPos = null;
            },
            onTick() {
                if (!gameState.injected || this.isBreaking) return;
                const playerEntity = gameUtils.getPlayerEntity();
                const breakingItem = playerEntity?._blockItem?.breakingItem;
                if (!breakingItem || breakingItem.attemptingToBreak) return;
                const playerPos = gameUtils.getPosition(1);
                if (!playerPos) return;
                if (this.bedPos) {
                    ...
                    if (!gameUtils.getBlockName(blockId)?.toLowerCase().includes('bed')) {
                        this.bedPos = null; this.pathCache = [];
                    }
                }
                if (!this.bedPos) {
                    const range = 6;
                    const x = Math.floor(playerPos[0]), y = Math.floor(playerPos[1] + 0.5), z = Math.floor(playerPos[2]);
                ...
                if (!this.bedPos || this.pathCache.length === 0) return;
                let target = null;
                for (const pos of this.pathCache) {
                    ...
                    if (blockId !== 0) {
                        ...
                        if (dist <= 7) { target = pos; break; }
                    }
                }
                if (!target) {
                    this.bedPos = null; this.pathCache = []; return;
                }
            ...
            resetBreaking(breakingItem, blockItem, key, original, originalTargeted) {
                Object.defineProperty(blockItem, key, { value: original, writable: true, configurable: true, enumerable: true });
                Object.defineProperty(breakingItem, "targetedPosition", { value: originalTargeted, writable: true, configurable: true, enumerable: true });
                try { breakingItem.resetTargetedBlock(Date.now()); } catch (err) {}
                breakingItem.attemptingToBreak = false;
                this.isBreaking = false;
            }
        });

        client.modules['Blink'] = new Module('Blink','World',{
            queued: [], colyRoom: null, sendBytesName: null, originalSendBytes: null, hookedProto: null,
            onEnable: function() {
                if (!this.colyRoom) {
                ...
                if (!this.colyRoom || !this.sendBytesName || !proto?.[this.sendBytesName]) return;
                ...
                proto[this.sendBytesName] = (...args) => {
                    if (this.enabled) { this.queued.push(args); } else { return this.originalSendBytes.apply(this.colyRoom, args); }
                };
            },
            onDisable: function() {
                if (this.originalSendBytes && this.hookedProto) {
                    for (const args of this.queued) this.originalSendBytes.apply(this.colyRoom, args);
        ...
        client.modules['Chest Stealer'] = new Module('Chest Stealer','World',{
            settings: { 'Delay': { type: 'slider', min: 0, max: 100, step: 1, value: 0 } },
            onEnable: function() {
                this.setLoop(async () => {
                    const inventory = gameState.noa.entities[gameState.impKey]?.inventory?.list?.[0]?.opWrapper;
                    if (!inventory) return;
                    ...
                    if (!chestInventory?.items || !playerInventory?.items) return;
                    const Op = 51, dg = 999;
                    ...
                    for (let i = 0; i < chestInventory.items.length; i++) {
                        await new Promise(resolve => setTimeout(resolve, this.settings['Delay'].value));
                        ...
                        if (item) {
                            if (item.typeObj?.stackable) {
                                ...
                                for (let j = 0; j < 46 && remainingCount > 0; j++) {
                                    ...
                                    if (!playerItem || playerItem.name !== itemName) continue;
                                    const currentStack = getItemAmount(playerItem);
                                    if (currentStack >= dg) continue;
                            ...
                            for (let j = 0; j < 46; j++) {
                                if (!playerInventory.items[j]) {
                                    inventory.swapPosClient(51 + i, j); break;
                                }
                            }
                        }
                    }
        ...
        client.modules['Inventory Manager'] = new Module('Inventory Manager','World',{
            settings: { 'Delay': { type: 'slider', min: 0, max: 100, step: 1, value: 0 } },
            interval: null, _mpqz: null,
            getWrapper: function() {
                try { return gameState.noa.entities[gameState.impKey]?.inventory?.list?.[0]?.opWrapper ?? null; } catch { return null; }
            },
            getMaterialRank: function(item) {
                if (!item?.name) return -1;
                const rank = { Wood: 0, Stone: 1, Iron: 2, Gold: 3, Diamond: 4, Knight: 5 };
                const mat = Object.keys(rank).find(k => item.name.includes(k));
                return mat ? rank[mat] : -1;
            },
            cleanInventory: function(wrapper) {
                if (!wrapper || !wrapper.playerInventory || !wrapper.playerInventory.items) return;
                const inv = wrapper.playerInventory, items = inv.items;
                const trash = ["Seeds", "Sapling", "Mushroom", "Empty Bottle"];
                const armor = { Helmet: 46, Chestplate: 47, Gauntlets: 48, Leggings: 49, Boots: 50 };
                ...
                const getItemSlots = function(filter) {
                    const slots = [];
                    for (let i = 0; i <= 45; i++) {
                        if (items[i] && filter(items[i])) slots.push(i);
                    }
                    return slots;
                };
                const setBestItem = function(name, targetSlot) {
                    const slots = getItemSlots(function(it) { return it.name.includes(name); });
                    if (!slots.length) return;
                    let best = -1, maxRank = -1;
                    for (const s of slots) {
                        const r = this.getMaterialRank(items[s]);
                        if (r > maxRank) { maxRank = r; best = s; }
                    }
                ...
                setBestItem("Sword", 0); setBestItem("Pickaxe", 2); setBestItem("Bow", 3);

                const bread = getItemSlots(function(it) { return it.name.includes("Bread"); });
                if (bread.length) wrapper.swapPosClient(bread[0], 8);

                let plankCnt = 0, stoneCnt = 0;
                const planks = getItemSlots(function(it) { return it.name.includes("Maple Wood Planks"); });
                const stones = getItemSlots(function(it) { return it.name.includes("Messy Stone"); });
                planks.forEach(function(s) { plankCnt += items[s].amount || 1; });
                stones.forEach(function(s) { stoneCnt += items[s].amount || 1; });
                ...
                for (const type of stackTypes) {
                    const stacks = getItemSlots(function(it) { return it.name.includes(type) && it.typeObj?.stackable; });
                    if (stacks.length <= 1) continue;
                    const tgt = stacks[0];
                    for (let i = 1; i < stacks.length; i++) {
                        const s = stacks[i];
                        if (!items[s]) continue;
                        try { wrapper.moveItemIntoIdxsClient(tgt, tgt + 1, s, items[s].amount); }
                        catch { wrapper.swapPosClient(s, tgt); }
                    }
                }
            },
            onEnable: function() {
                ...
                this.interval = setInterval(function() {
                    ...
                    if (w) this.cleanInventory(w);
                }.bind(this), this.settings['Delay'].value || 100);
            },
            onDisable: function() {
        ...
        client.modules['Auto Respawn'] = new Module('Auto Respawn', 'World', {
            isRespawning: false,
            onTick: function() {
                const lifeState = gameState.noa.entities.getState(1, 'genericLifeformState');
                if (lifeState && !lifeState.isAlive) {
                    if (!this.isRespawning) {
                        this.isRespawning = true;
                        setTimeout(() => {
                            const buttons = document.querySelectorAll('.ButtonBody, .FullyFancyText');
                            for (let i = 0; i < buttons.length; i++) {
                                const text = buttons[i].textContent.toLowerCase();
                                if (text.includes("respawn") || text.includes("hồi sinh")) {
                                    buttons[i].click();
                                    break;
                                }
                            }
        ...
        client.modules['New Account'] = new Module('New Account','World',{
            type: 'action',
            onEnable: function() {
                if (window.confirm('Are you sure you want to reset your session and create a new account?')) {
